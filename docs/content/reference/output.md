...
describes what users get. Pick a format with `-o`, or let historicalweather choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
historicalweather <command> -o table   # aligned columns for reading
historicalweather <command> -o jsonl   # one JSON object per line, for piping
historicalweather <command> -o json    # a single JSON array
historicalweather <command> -o csv     # spreadsheet friendly
historicalweather <command> -o tsv     # tab-separated
historicalweather <command> -o url     # just the URL column
historicalweather <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
historicalweather <command> --fields id,title,url
```
...
```bash
historicalweather <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
historicalweather <command>            # a table, because this is a terminal
historicalweather <command> | wc -l    # JSONL, because this is a pipe
```
