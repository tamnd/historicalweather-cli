...
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 historicalweather

COPY $TARGETPLATFORM/historicalweather /usr/bin/historicalweather

USER historicalweather

ENTRYPOINT ["/usr/bin/historicalweather"]
